  ...
  header("Location: login.php?logout=si");
    ...
    if(!empty($_POST))
    {
        ...
        if ($isSuccess)
        {
          ...
          $statement = $db->prepare("UPDATE files  set name = ?, id_subject = ?, type_cat = ?, image= ? ,correction= ?  WHERE id = ?");
    ...
    <body>
        <h1 class="text-logo"><span class="glyphicon glyphicon-folder-open"></span> <span class="neretto">D</span>es <span class="neretto">A</span>nnales <span class="glyphicon glyphicon-folder-open" ></span></h1>
         <div class="container admin">
            <div class="row">
                <div class="col-sm-12">
                    <h1 style="text-align:center"><strong>Modifier un ficher</strong></h1>
                    <br>
                    <form class="form" action="<?php echo 'updateFic.php?id='.$id;?>" role="form" method="post" enctype="multipart/form-data">
