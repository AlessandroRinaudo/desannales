...
if(!empty($_POST))
{
  ...
  $isSuccess          = true;
    ...
    if($isSuccess)
    {
      ...
      {
        $statement = $db->prepare("INSERT INTO articoli (titolo,paragrafo) values(?, ?)");
        $statement->execute(array($titre,$paraghraphe));
        Database::disconnect();
      }
...
<body>
    <h1 class="text-logo"><span class="glyphicon glyphicon-folder-open"></span> <span class="neretto">D</span>es <span class="neretto">A</span>nnales <span class="glyphicon glyphicon-folder-open" ></span></h1>
     <div class="container admin">
        <div class="row">
            <h1 style="text-align: center"><strong>Ajouter un post</strong></h1>
            <br>
            <form class="form" action="insertPost.php" role="form" method="post" enctype="multipart/form-data">
